...
public class Math3d
{
	...
	public static Vector3 ProjectPointOnPlaneOrtho(Vector3 planeNormal, Vector3 planePoint, Vector3 point){
	...
	//not on the plane, use ProjectPointOnPlaneOrtho() on linePoint1 and linePoint2 first.
	...
	//plane as the rectangle. If the point is not on the plane, use ProjectPointOnPlaneOrtho() first.
	...
	public static Vector3 PointLineClosestPoint(Vector3 point, Vector3 linePoint, Vector3 lineDirection)
	{
		float d1 = Vector3.Dot( point-linePoint, lineDirection );
		float d2 = Vector3.Dot( lineDirection, lineDirection );
		
		return linePoint + lineDirection*(d1/d2);
	}
	
	// PointLineDistance
	public static float PointLineDistance(Vector3 point, Vector3 linePoint, Vector3 lineDirection)
	{
		return (PointLineClosestPoint(point, linePoint, lineDirection) - point).magnitude;
	}
	
	//This function returns a point which is a projection from a point to a plane.
	public static Vector3 ProjectPointOnPlane(Vector3 planeNormal, Vector3 planePoint, Vector3 point, Vector3 direction){
		...
		float dot = Vector3.Dot( direction.normalized, planeNormal );
		
		if (Mathf.Abs(dot) >= 0.000001f)
		{
			float h = SignedDistancePlanePoint(planeNormal, planePoint, point);
			return point + direction*(-h/dot);
		}
		return point;
	}		
}
