...
void main()
{
  ...
  //vec4 color = mix(texture2D(MainTexture, gl_TexCoord[0].xy), vec4(gl_TexCoord[0].x, gl_TexCoord[0].y, 1.0, 1.0), 0.5);
  vec4 color = texture2D(MainTexture, gl_TexCoord[0].xy);
  
  //if (length(fragPosition) < WaterLevel)
  //  color = vec4(0.0, 0.0, 1.0, 1.0);
 // color *= (length(fragPosition) / 25.0 - WaterLevel) / Amplitude;
  //shadow = 1.0 - shadow*0.01;
  vec3 L = normalize(gl_LightSource[0].position.xyz - fragPosition);
  vec4 Idiff = color * max(pow(dot(fragNormal, L), 0.5), 0.0);
